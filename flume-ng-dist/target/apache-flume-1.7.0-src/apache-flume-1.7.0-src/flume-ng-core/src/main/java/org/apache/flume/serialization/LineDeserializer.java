...
public class LineDeserializer implements EventDeserializer {
	...
	public static final String MAXEVENTSIZE = "eventSize";
	public static final int DEFAULTMAXEVENTSIZE = 1048576;
	...
	LineDeserializer(Context context, ResettableInputStream in) {
		...
		this.eventSize = context.getInteger(MAXEVENTSIZE, DEFAULTMAXEVENTSIZE);
		this.isOpen = true;
	}
	...
	public Event readEvent() throws IOException {
		...
		l += 1;
		while (body.getBytes(outputCharset).length < eventSize) {
			...
			if (line == null) {
				...
				logger.info("spooldir thread " + Thread.currentThread().getId() + " read " + l + " lines" + " sizes = "
						+ body.getBytes(outputCharset).length + " successfully usetime = " + (endTime - beginTime));
				return EventBuilder.withBody(body, outputCharset);
			} else {
				l += 1;
				body += "\n";
				body += line;
