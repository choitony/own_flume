...
public abstract class AbstractConfigurationProvider implements ConfigurationProvider {
			...
			try {
				...
				for (String channelName : channelNames) {
					...
					if (channelComponent.components.isEmpty()) {
						...
						LOGGER.info(String.format("Channel %s connected to %s", channelName,
								channelComponent.components.toString()));
	...
	private void loadChannels(AgentConfiguration agentConf, Map<String, ChannelComponent> channelComponentMap)
		...
		/*
		 * Some channels will be reused across re-configurations. To handle
		 * this, we store all the names of current channels, perform the
		 * reconfiguration, and then if a channel was not used, we delete our
		 * reference to it. This supports the scenario where you enable channel
		 * "ch0" then remove it and add it back. Without this, channels like
		 * memory channel would cause the first instances data to show up in the
		 * seconds.
		 */
		...
		/*
		 * Components which DO NOT have a ComponentConfiguration object and use
		 * only Context
		 */
		for (String chName : channelNames) {
	...
	private Channel getOrCreateChannel(ListMultimap<Class<? extends Channel>, String> channelsNotReused, String name,
			String type) throws FlumeException {
	...
	private void loadSources(AgentConfiguration agentConf, Map<String, ChannelComponent> channelComponentMap,
			Map<String, SourceRunner> sourceRunnerMap) throws InstantiationException {
		...
		for (String sourceName : sourceNames) {
			...
			if (context != null) {
				...
				try {
					...
					Map<String, String> selectorConfig = context
							.getSubProperties(BasicConfigurationConstants.CONFIG_SOURCE_CHANNELSELECTOR_PREFIX);
	...
	private void loadSinks(AgentConfiguration agentConf, Map<String, ChannelComponent> channelComponentMap,
			Map<String, SinkRunner> sinkRunnerMap) throws InstantiationException {
		LOGGER.info("loadSinks >>> ");
		...
		for (String sinkName : sinkNames) {
			...
			if (comp != null) {
				...
				LOGGER.info("configuration type = "+ comp.getType());
				LOGGER.info("configuration = "+ comp);
		...
		for (String sinkName : sinkNames) {
			...
			if (context != null) {
				...
				// modified by wentao chai
				LOGGER.info("sink type = " + context.getString(BasicConfigurationConstants.CONFIG_TYPE));
				LOGGER.info("context = " + context);

				int sinkNum = context.getInteger("sinknum", 1);
				if (sinkNum == 0) {
					LOGGER.error("sink = " + sinkName + " sinknum can not be 0");
				}
				LOGGER.info("sink = "+sinkName+" configure "+sinkNum +" subSinks");
				for (int i = 0; i < sinkNum; i++) {
					String subSinkName = sinkName + "-" + i;
					Sink sink = sinkFactory.create(subSinkName,
							context.getString(BasicConfigurationConstants.CONFIG_TYPE));
					try {
						...
						sink.setChannel(channelComponent.channel);
						sinks.put(subSinkName, sink);
						channelComponent.components.add(subSinkName);
					} catch (Exception e) {
						String msg = String.format("Sink %s has been removed due to an " + "error during configuration",
								sinkName);
						LOGGER.error(msg, e);
	...
	private void loadSinkGroups(AgentConfiguration agentConf, Map<String, Sink> sinks,
			Map<String, SinkRunner> sinkRunnerMap) throws InstantiationException {
