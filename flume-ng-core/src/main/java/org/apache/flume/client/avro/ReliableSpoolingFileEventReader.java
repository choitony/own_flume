...
import org.apache.flume.source.CertError;
...
public class ReliableSpoolingFileEventReader implements ReliableEventReader {
	...
	public static final HashSet<String> currentFileList = new HashSet<String>();
	public static List<File> candidateFiles = Collections.emptyList();
	
	public static final Object currentFileListLock = new Object();
	public static final Object  getNextFileLock =  new Object();
	public static Iterator<File> candidateFileIter = null;
	private static int listFilesCount = 0;
	...
	private boolean iscurrentFile(String fileKey){
		if(fileKey == null)
			return false;
		synchronized(currentFileListLock){
			boolean result = currentFileList.contains(fileKey);
			
			return  result;
		}
	}
	
	private boolean addcurrentFile(String fileKey){
		if(fileKey == null)
			return false;
		synchronized(currentFileListLock){
			boolean result = currentFileList.add(fileKey);
			logger.info("test threadId " + threadId + " add file " + fileKey + " " + result + " all: " + currentFileList.toString());
			return result;
		}

	}
	
	private boolean removecurrentFile(String fileKey){
		if(fileKey == null)
			return true;
		synchronized(currentFileListLock){
			boolean result = currentFileList.remove(fileKey);
			return result;
		}
	}
	/**
	...
	private ReliableSpoolingFileEventReader(File spoolDirectory, String completedSuffix, String ignorePattern,
			...
			String inputCharset, DecodeErrorPolicy decodeErrorPolicy, ConsumeOrder consumeOrder, int threadId)
			throws IOException {
		...
		try {
			File canary = File.createTempFile("flume-spooldir-perm-check-" + threadId , ".canary", spoolDirectory);
		...
		this.metaFile = new File(trackerDirectory, metaFileName + threadId);
	...
	private void rollCurrentFile(File fileToRoll) throws IOException {
		...
		String failedSuffix = completedSuffix;
		File dest = new File(fileToRoll.getPath() + failedSuffix);
		...
		if (dest.exists() && PlatformDetect.isWindows()) {
			...
			// Destination file does not already exist. We are good to go!
				...
				// now we no longer need the meta file
					if(!removecurrentFile(fileToRoll.getAbsolutePath())){
						logger.error("Unable to delete file " + fileToRoll.getAbsolutePath() + "from currentFileList. It may not exitied");
					}
					deleteMetaFile();
				} else {
				/*
	...
	//	List<File> candidateFiles = Collections.emptyList();
		...
		synchronized(getNextFileLock){
				...
				FileFilter filter = new FileFilter() {
					public boolean accept(File candidate) {
						...
						if ((candidate.isDirectory()) 
							...
							|| (fileName.endsWith(".canary"))
							|| (fileName.startsWith(".")) 
							|| ignorePattern.matcher(fileName).matches()
							|| (iscurrentFile(candidate.getAbsolutePath()))) {
							return false;
						}else{
							return true;
						}
			...
			selectedFile = candidateFileIter.next();
			...
			addcurrentFile(selectedFile.getAbsolutePath());
		}
		logger.debug("test thread " + threadId + "will read file: " + selectedFile.getName());
		return openFile(selectedFile);
	}
	...
	private Optional<FileInfo> openFile(File file) {
		try {
			...
			removecurrentFile(file.getAbsolutePath());
			return Optional.absent();
		} catch (IOException e) {
	...
	public static class Builder {
		...
		public ReliableSpoolingFileEventReader build() throws IOException {
			return new ReliableSpoolingFileEventReader(spoolDirectory, completedSuffix, ignorePattern, trackerDirPath,
					...
					deserializerContext, deletePolicy, inputCharset, decodeErrorPolicy, consumeOrder, threadId);
		}
	}
